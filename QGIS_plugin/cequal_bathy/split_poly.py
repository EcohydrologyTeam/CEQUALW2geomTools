...
def splitPolygon_func(self, transectWidth):
    ...
    #split centerline by segments
    params={ 'INPUT' : cl_lyr, 'INPUT_FIELDS' : [], 'OUTPUT' : 'TEMPORARY_OUTPUT', 'OVERLAY' : seg_lyr, 'OVERLAY_FIELDS' : [], 'OVERLAY_FIELDS_PREFIX' : '' }
    result_cl = processing.run("native:intersection", params)
    result_layer_cl_seg = result_cl['OUTPUT']
    ...
    #join to get segment id
    params={ 'INPUT' : result_layer_cl_seg, 'INPUT_FIELDS' : [], 'OUTPUT' : 'TEMPORARY_OUTPUT', 'OVERLAY' : QgsProcessingFeatureSourceDefinition(seg_lyr.id(), True), 'OVERLAY_FIELDS' : [], 'OVERLAY_FIELDS_PREFIX' : '' }
    result_cl_seg = processing.run("native:intersection", params)
    result_layer_cl_seg = result_cl_seg['OUTPUT']
